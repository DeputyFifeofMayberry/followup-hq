...
export function UniversalIntakeWorkspace() {
  ...
  const handleDecision = (decision: 'approve_followup' | 'approve_task' | 'reference' | 'reject' | 'link') => {
    ...
    if (decision === 'link') {
      ...
      return setFeedback({ tone: 'success', message: `Linked to ${selectedMatch.recordType} ${selectedMatch.id}.` });
    }
  ...
  return (
    <div className="space-y-3">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-3">
        ...
        <div className="mt-2"><button className="action-btn" onClick={() => { void ingestIntakeText(manualText, 'Pasted note'); setManualText(''); }}>Create intake candidate from pasted text</button></div>
      </section>
        ...
        <div className="space-y-2">
          {activeBatches.slice(0, 8).map((batch) => {
            ...
            return <div key={batch.id} className="rounded-lg border border-slate-200 p-2 text-xs">
              ...
              <div className="text-slate-600">Files {batchAssets.length} • Candidates {batchCandidates.length} • Failures {failures.length} • Status {batch.status}</div>
              ...
              {failures.map((asset) => <div key={asset.id} className="mt-1 rounded border border-amber-200 bg-amber-50 px-2 py-1">{asset.fileName}: {asset.errors[0] || asset.warnings[0]}
                <div className="mt-1 flex gap-1"><button className="action-btn !px-2 !py-0.5 text-[11px]" onClick={() => void retryIntakeAssetParse(asset.id)}>Retry parse</button><button className="action-btn !px-2 !py-0.5 text-[11px]" onClick={() => removeIntakeAsset(asset.id)}>Remove failed asset</button></div>
              </div>)}
            </div>;
          })}
          {!activeBatches.length ? <div className="text-xs text-slate-500">No active batches.</div> : null}
          {archivedBatches.length ? <div className="text-xs text-slate-500">Archived: {archivedBatches.length}</div> : null}
        </div>
      </section>
      ...
      <section className="grid gap-3 lg:grid-cols-12">
        ...
        <div className="lg:col-span-3 rounded-xl border border-slate-200 bg-white p-3">
          ...
          {selectedAsset ? <div className="space-y-2 text-xs">
            ...
            {selectedSourceTab === 'preview' ? <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 max-h-[420px] overflow-auto whitespace-pre-wrap">{selectedAsset.extractedPreview || selectedAsset.extractedText.slice(0, 2000) || 'No preview extracted.'}</div> : null}
            {selectedSourceTab === 'evidence' ? <div className="space-y-1.5">{(selectedCandidate?.evidence || []).map((entry) => <div key={entry.id} className="rounded border border-slate-200 bg-slate-50 p-2"><div className="text-[11px] uppercase tracking-wide text-slate-500">{entry.field} • {entry.locator || entry.sourceRef}</div><div>{sanitizeSnippet(entry.snippet).slice(0, 220)}</div>{entry.locator?.includes('#row') ? <div className="mt-1 inline-flex items-center gap-1 rounded bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600"><FileSpreadsheet className="h-3 w-3" />{entry.locator}</div> : null}</div>)}</div> : null}
