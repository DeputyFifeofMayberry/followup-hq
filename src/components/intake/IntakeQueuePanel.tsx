...
export function IntakeQueuePanel({ activeLane, byLane, laneCounts, selectedCandidateId, onSelectCandidate, onSetLane }: Props) {
  ...
  return (
    <section className="intake-lane-panel">
      ...
      <div className="mb-2 intake-lane-tabs">{(['needs_correction', 'link_duplicate_review', 'ready_to_create', 'reference_only'] as QueueLane[]).map((lane) => <button key={lane} className={`action-btn intake-lane-tab !px-2 !py-1 text-xs ${activeLane === lane ? '!border-sky-300 !bg-sky-50 !text-sky-700' : ''}`} onClick={() => onSetLane(lane)}>{laneLabel(lane)} • {laneCounts[lane]}</button>)}</div>
      <div className="space-y-2 max-h-[65vh] overflow-auto">{visible.map((queueItem) => {
        const blockerCount = queueItem.missingCriticalFields;
        return <button key={queueItem.id} className={`w-full rounded-xl border p-3 text-left ${selectedCandidateId === queueItem.id ? 'border-sky-300 bg-sky-50' : 'border-slate-200 bg-white'}`} onClick={() => onSelectCandidate(queueItem.id)}>
