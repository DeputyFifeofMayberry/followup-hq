...
> {
  return {
    ...
    deleteItem: (id) => {
      ...
      get().pushToast({ tone: 'warning', kind: 'undo_offer', title: 'Follow-up deleted', source: 'followups.deleteItem', recordType: 'followup', recordIds: [id], action: { label: 'Undo', actionId: 'undo-followup-delete' } });
      queuePersist({ dirtyRecords: [{ type: 'followup', id }] });
    },
    ...
    runValidatedBatchFollowUpTransition: (ids, status, patch = {}, options) => {
      ...
      get().pushToast({ kind: 'bulk_result', tone: feedback.tone, title: feedback.title, message: feedback.message, source: 'followups.runValidatedBatchFollowUpTransition', recordType: 'followup', recordIds: ids, operationSummary: { affected, skipped, warnings } });
    ...
    importItems: (rows) => { set((state: AppStore) => { const imported = rows.map(buildImportedItem); const items = [...imported, ...state.items].map(normalizeItem); return { items, selectedId: imported[0]?.id ?? state.selectedId, importModalOpen: false, duplicateReviews: refreshDuplicates(items, state.dismissedDuplicatePairs) }; }); get().pushToast({ kind: 'bulk_result', tone: 'success', title: `${rows.length} follow-ups imported`, source: 'followups.importItems', recordType: 'followup', recordIds: rows.map((row) => row.id), operationSummary: { affected: rows.length, skipped: 0, warnings: [] } }); queuePersist({ dirtyRecords: rows.map((row) => ({ type: 'followup' as const, id: row.id })) }); },
    ...
    batchUpdateFollowUps: (ids, patch, summary) => {
      ...
      get().pushToast({ kind: 'bulk_result', tone: 'success', title: `${ids.length} follow-ups updated`, source: 'followups.batchUpdateFollowUps', recordType: 'followup', recordIds: ids, operationSummary: { affected: ids.length, skipped: 0, warnings: [] } });
      queuePersist();
    },
    ...
    confirmFollowUpSent: (id, notes) => { set((state: AppStore) => ({ items: withItemUpdate(state.items, id, (item) => ({ ...item, actionState: 'Sent (confirmed)', status: 'Waiting on external', lastTouchDate: todayIso(), nextTouchDate: addDaysIso(todayIso(), item.cadenceDays || 3), lastCompletedAction: 'Sent follow-up (confirmed)', lastActionAt: todayIso(), actionReceipts: [{ id: createId('ACT'), at: todayIso(), actor: 'Current user', action: 'send_confirmed', confirmed: true, notes }, ...(item.actionReceipts || [])], timeline: [buildTouchEvent('Send confirmed by user in composer.', 'bundle_action'), ...item.timeline] })) })); get().pushToast({ tone: 'success', title: 'Follow-up marked as sent', source: 'followups.confirmFollowUpSent', recordType: 'followup', recordIds: [id] }); queuePersist({ dirtyRecords: [{ type: 'followup', id }] }); },
  };
}
