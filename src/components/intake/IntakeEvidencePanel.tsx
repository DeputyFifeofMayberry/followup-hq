...
export function IntakeEvidencePanel(props: Props) {
  ...
  return (
    <section className="intake-guidance-card intake-evidence-panel">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div>
          <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Evidence (secondary)</div>
          <div className="text-xs text-slate-600">Use evidence only when a decision needs verification.</div>
        </div>
      </div>
      <div className="mb-2 flex gap-1.5">{tabs.map((tab) => <button key={tab} className={`action-btn !px-2 !py-1 text-xs ${props.selectedSourceTab === tab ? '!border-sky-300 !bg-sky-50 !text-sky-700' : ''}`} onClick={() => props.onSetTab(tab)}>{tab}</button>)}</div>
      {props.selectedAsset ? <div className="space-y-2 text-xs max-h-[320px] overflow-auto">
        ...
        {props.selectedSourceTab === 'evidence' ? <div className="space-y-1.5">{(props.selectedCandidate?.evidence || []).map((entry) => <button key={entry.id} className="w-full rounded border border-slate-200 bg-slate-50 p-2 text-left" onClick={() => props.onSelectLocator(entry.locator || entry.sourceRef)}><div className="text-[11px] uppercase tracking-wide text-slate-500">{entry.field} • {entry.locator || entry.sourceRef}</div><div>{sanitizeSnippet(entry.snippet).slice(0, 220)}</div>{entry.locator?.includes('#row') ? <div className="mt-1 inline-flex items-center gap-1 rounded bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600"><FileSpreadsheet className="h-3 w-3" />{entry.locator}</div> : null}</button>)}</div> : null}
