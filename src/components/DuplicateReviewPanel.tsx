...
export function DuplicateReviewPanel() {
  const { items, duplicateReviews, setSelectedId, dismissDuplicatePair, openMergeModal, followUpDuplicateModule } = useAppStore(useShallow((s) => ({
    items: s.items,
  ...
  const isExpanded = followUpDuplicateModule === 'expanded'
    || (followUpDuplicateModule === 'auto' ? (selectedHasDuplicates || expanded) : false);
