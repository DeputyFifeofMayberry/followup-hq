...
export function IntakeEvidencePanel(props: Props) {
  const tabs: SourceTab[] = ['overview', 'receipt', 'preview', 'evidence', 'metadata'];
  const receipt = props.selectedAsset?.parserReceipt;
  const nextStepLabel: Record<string, string> = {
    create_action_after_review: 'Continue triage in actionable queue',
    review_extracted_source: 'Review extracted source text',
    interpret_manually: 'Interpret manually before creating work',
    retry_parse: 'Retry parse from original upload bytes',
    remove_failed_asset: 'Remove failed source from batch',
    save_as_reference: 'Preserve as reference material',
  ...
  return (
    <section className="intake-evidence-panel">
      ...
      {props.selectedAsset ? <div className="space-y-2 text-xs max-h-[320px] overflow-auto">
        {props.selectedSourceTab === 'overview' ? <div className="rounded-lg border border-slate-200 bg-slate-50 p-2"><div className="font-semibold text-slate-800">{props.selectedAsset.fileName}</div><div className="text-slate-600">{prettyFileSize(props.selectedAsset.sizeBytes)} • {props.selectedAsset.kind}</div><div className="mt-1 text-[11px] text-slate-600">Parser path: {receipt?.parserPath || 'standard_parse'} • Admission: {props.selectedAsset.admissionState || 'reviewable'}</div></div> : null}
        {props.selectedSourceTab === 'receipt' ? <div className="rounded-lg border border-slate-200 bg-slate-50 p-2 space-y-1.5">
          <div><strong>Capability:</strong> {receipt?.capabilityClass || 'unknown'}</div>
          <div><strong>Recovered content:</strong> {receipt?.recoveredContentStatus || 'unknown'} • <strong>Route:</strong> {receipt?.weakSourceRoute || 'unknown'}</div>
          <div><strong>Downgrade reasons:</strong> {(receipt?.downgradeReasons?.length ? receipt.downgradeReasons : ['None']).join(' · ')}</div>
          <div><strong>Warnings:</strong> {(receipt?.warningSummary?.length ? receipt.warningSummary : ['None']).join(' · ')}</div>
          <div><strong>Next steps:</strong> {(receipt?.userNextSteps?.length ? receipt.userNextSteps.map((step) => nextStepLabel[step] || step) : ['Continue review flow']).join(' · ')}</div>
        </div> : null}
