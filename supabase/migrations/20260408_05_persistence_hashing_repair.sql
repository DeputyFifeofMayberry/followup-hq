-- Canonical persistence hashing repair.
-- Removes digest()/pgcrypto dependency from persistence RPCs and exposes explicit hashing-failure diagnostics.

insert into public.persistence_contract_meta (contract_name, contract_version, migration_signature, updated_at)
values ('entity_persistence_v2', '2026-04-08.5', '20260408_05_persistence_hashing_repair', now())
...
begin
  if exists (select 1 from information_schema.tables where table_schema = 'public' and table_name = 'persistence_contract_meta') then
    select pcm.contract_version, (pcm.migration_signature = '20260408_05_persistence_hashing_repair')
  ...
  if apply_save_batch_def is not null and position('digest(' in lower(apply_save_batch_def)) > 0 then
    return jsonb_build_object(
