import { CheckCircle2, Link2, Pencil, Plus, Search, Trash2, Undo2 } from 'lucide-react';
    ...
    <div className="space-y-5">
      ...
      <div className="workspace-master-detail">
        <AppShellCard className="workspace-list-panel">
          <SectionHeader title="Tasks" subtitle="Execution review surface with PM-grade workflow filters." actions={<button onClick={openCreateTaskModal} className="primary-btn"><Plus className="h-4 w-4" />Add task</button>} />
          <FilterBar>
            <SegmentedControl value={mode} onChange={(value) => setMode(value as TaskMode)} options={modeOptions} />
          </FilterBar>

          <div className={`grid gap-3 ${personalMode ? 'md:grid-cols-3' : 'md:grid-cols-4'}`}>
                ...
                <button key={task.id} onClick={() => setSelectedTaskId(task.id)} className={`workspace-data-row workspace-data-row-compact ${selectedTask?.id === task.id ? 'workspace-data-row-active' : ''}`}>
                  <div className="workspace-data-row-main">
                    <div>
                      ...
                      {isTaskDeferred(task) ? <div className={`mt-1 text-xs ${selectedTask?.id === task.id ? 'text-indigo-200' : 'text-indigo-700'}`}>Deferred until {formatDate(task.deferredUntil)}</div> : null}
                    </div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    ...
                    {task.completionImpact ? <Badge variant="neutral">Impact: {task.completionImpact}</Badge> : null}
            ...
            <div className="space-y-5">
              ...
              <div className="grid gap-2">
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => updateTask(selectedTask.id, { status: 'Done', completionNote: selectedTask.completionNote || 'Completed from inspector.' })} className="action-btn">Mark done</button>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button onClick={() => updateTask(selectedTask.id, { completionImpact: selectedTask.completionImpact === 'close_parent' ? 'advance_parent' : 'close_parent' })} className="action-btn">Toggle completion impact</button>
                </div>
              </div>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <div className="rounded-2xl bg-slate-50 p-3"><div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Impact</div><div className="mt-2 text-sm font-medium text-slate-900">{selectedTask.completionImpact || 'advance_parent'}</div></div>
                <div className="rounded-2xl bg-slate-50 p-3"><div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Start</div><div className="mt-2 text-sm font-medium text-slate-900">{formatDate(selectedTask.startDate)}</div></div>
                <div className="rounded-2xl bg-slate-50 p-3"><div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Due</div><div className="mt-2 text-sm font-medium text-slate-900">{formatDate(selectedTask.dueDate)}</div></div>
                <div className="rounded-2xl bg-slate-50 p-3"><div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Deferred until</div><div className="mt-2 text-sm font-medium text-slate-900">{formatDate(selectedTask.deferredUntil)}</div></div>
                <div className="rounded-2xl bg-slate-50 p-3"><div className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Block reason</div><div className="mt-2 text-sm font-medium text-slate-900">{selectedTask.blockReason || '—'}</div></div>
              </div>
