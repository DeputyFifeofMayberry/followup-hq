    ...
    <div className="grid gap-4 xl:grid-cols-[1.6fr,1fr]">
      <section className="rounded-2xl border border-slate-200 bg-white p-4">
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          <select className="field-input max-w-44" value={vm.projectFilters.owner} onChange={(event) => vm.setProjectFilters((prev: ProjectFilterState) => ({ ...prev, owner: event.target.value }))}><option value="All">All owners</option>{[...new Set(vm.projects.map((project: any) => project.owner))].filter(Boolean).map((owner) => <option key={String(owner)}>{String(owner)}</option>)}</select>
