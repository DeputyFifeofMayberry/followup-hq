...
import { CheckCircle2, FileEdit, Link2, Send, SquareCheckBig, Trash2 } from 'lucide-react';
...
export function ItemDetailPanel({ personalMode = false, inModal = false, onRequestClose }: { personalMode?: boolean; inModal?: boolean; onRequestClose?: () => void }) {
  const {
    items, tasks, projects, contacts, companies, updateItem, deleteItem, openRecordEditor, addTask, addTouchLog, openDraftModal, setSelectedTaskId,
    openRecordDrawer, attemptFollowUpTransition, isRecordDirty, search, activeView, followUpFilters, selectedId, setSelectedId,
  } = useAppStore(useShallow((s) => ({
    items: s.items, tasks: s.tasks, projects: s.projects, contacts: s.contacts, companies: s.companies, updateItem: s.updateItem, deleteItem: s.deleteItem,
    openRecordEditor: s.openRecordEditor, addTask: s.addTask, addTouchLog: s.addTouchLog, openDraftModal: s.openDraftModal, setSelectedTaskId: s.setSelectedTaskId,
  ...
  const [detailView, setDetailView] = useState<'overview' | 'activity' | 'details'>('overview');
  ...
  const content = (
    <>
      ...
      <SegmentedControl
        ...
        options={[
          { value: 'overview', label: 'Work item' },
      ...
      {detailView === 'overview' ? <div className="detail-card inspector-block">
        ...
        <div className="mt-2 text-sm text-slate-700">{recommendedAction?.reason ?? laneContext.nextMove?.reason ?? 'Move this record forward with one clear action.'}</div>
      </div> : null}

      {detailView === 'activity' ? <details className="detail-card inspector-block mt-3" open>
      ...
      {detailView === 'details' ? <div className="space-y-3">
        <details className="detail-card inspector-block" open>
          ...
          <div className="mt-3 space-y-3">
            ...
            {closeout ? <CloseoutReadinessCard evaluation={closeout} onAddCompletionNote={() => setActiveAction('close')} onOpenTask={(taskId) => { setSelectedTaskId(taskId); openRecordDrawer({ type: 'task', id: taskId }); }} onReviewLinkedRecords={() => openRecordDrawer({ type: 'followup', id: item.id })} /> : null}
