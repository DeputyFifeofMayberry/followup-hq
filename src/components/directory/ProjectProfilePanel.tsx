...
import { PROJECT_STATUS_OPTIONS, splitLocationValue, toDelimitedArray } from '../../domains/directory/hooks/useDirectoryViewModel';
...
export function ProjectProfilePanel({ selectedRow, editing, draft, detailTab, saveErrors, onSetEditing, onDraftChange, onSaveDraft, onSetDetailTab, onArchiveToggle, onDelete, onOpenFollowUp, onOpenTask, onOpenDirectoryRecord }: ProjectProfilePanelProps) {
    ...
    <>
      <div className="mb-3 flex items-start justify-between gap-2">
        ...
        <div className="flex gap-2">
          ...
          {editing ? <><button className="action-btn" onClick={() => onSetEditing(false)}>Cancel</button><button className="primary-btn" onClick={onSaveDraft}>Save</button></> : null}
        </div>
      </div>
      ...
      {detailTab === 'profile' ? (
        ...
        <div className="mt-3 rounded-xl border border-slate-200 p-3 text-sm">
          ...
          <div className="mt-2 text-xs text-slate-600">Latest activity: {formatDate(selectedRow.updatedAt)}</div>
          <div className="mt-1 text-xs text-slate-600">Next action: {selectedRow.project.projectNextAction || 'Not set'}</div>
          <div className="mt-1 text-xs text-slate-600">Closeout readiness: {selectedRow.project.closeoutReadiness ?? 0}%</div>
          <div className="mt-2 text-xs text-slate-700">Top overdue/open follow-ups</div>
          <div className="space-y-1">{selectedRow.openFollowUps.slice(0, 3).map((item: any) => <button key={item.id} className="w-full rounded border border-slate-200 px-2 py-1 text-left text-xs" onClick={() => onOpenFollowUp(item.id)}>{item.title}</button>)}</div>
          <div className="mt-2 text-xs text-slate-700">Top blocked tasks</div>
          <div className="space-y-1">{selectedRow.openTasks.filter((task: any) => task.status === 'Blocked').slice(0, 3).map((task: any) => <button key={task.id} className="w-full rounded border border-slate-200 px-2 py-1 text-left text-xs" onClick={() => onOpenTask(task.id)}>{task.title}</button>)}</div>
          <div className="mt-3 flex gap-2"><button className="action-btn" onClick={() => selectedRow.openFollowUps[0] && onOpenFollowUp(selectedRow.openFollowUps[0].id)}>Open project follow-ups</button><button className="action-btn" onClick={() => selectedRow.openTasks[0] && onOpenTask(selectedRow.openTasks[0].id)}>Open project tasks</button></div>
          <div className="mt-2 space-y-1">{selectedRow.health.reasons.map((reason: string) => <div key={reason} className="text-xs text-slate-700">• {reason}</div>)}</div>
