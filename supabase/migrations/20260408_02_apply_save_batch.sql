-- RPC creation migration: authoritative save-batch endpoint for client cloud persistence.
...
begin
  ...
  v_server_payload_hash := encode(digest(jsonb_build_object(
    'batchId', v_batch_id,
...
-- Verification:
-- 1) Confirm entity contract columns:
--    select column_name from information_schema.columns
--    where table_schema = 'public' and table_name = 'follow_up_items'
--    order by ordinal_position;
-- 2) Confirm RPC exists:
--    select proname, oidvectortypes(proargtypes)
--    from pg_proc p join pg_namespace n on n.oid = p.pronamespace
--    where n.nspname = 'public' and p.proname = 'apply_save_batch';
